...
- Select your `AWS region` as the current region where you are doing this workshop. Not sure? As your instructor. (Example: `us-east-1`)
- The bucket name has to be unique. Enter `bucket name` as `cloudtrail-\<aws-region\>-\<aws-account-number\>` (Example: If your region is `us-east-1` and your AWS account id is `123456789012` then enter `cloudtrail-us-east-1-123456789012` as bucket name)
